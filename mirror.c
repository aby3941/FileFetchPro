...
void processclient(int csd1){
    ...
    if(!((client_connection_count > 3 && client_connection_count < 8) || (client_connection_count > 7 && ((client_connection_count % 2) == 1))))
    {
        exit(0);
    }
    int n, pid;
    write(csd1, "Connected to mirror server", strlen("Connected to mirror server"));
